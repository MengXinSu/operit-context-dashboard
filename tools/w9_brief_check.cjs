#!/usr/bin/env node
/**
 * W9① 步 brief 结构自检（升级自 w7_brief_check）：从桥提取 W7_BRIEF 段 + fa2Unesc/fa2ToolTail，
 * 对全库 raw_*.json 逐步单步现算（w7BriefOne），验证：
 *   1) 每步 op/ins/res 下标均为合法 hist 下标；
 *   2) ins 条目 kind 全为 TOOL_RESULT；res 条目 kind ∈ {ASSISTANT, TOOL_CALL}；op 为 USER；
 *   3) 下标顺序严格递增；越界 s 返回 null；pIdx 与请求点表一致；
 *   4) 输出统计（steps/opener 缺失/预览空率）。
 * 用法：node tools/w9_brief_check.cjs [index.ui.js] [prompt_viewer_dir]
 * PASS=退出码 0。
 */
...
const api = new Function(code + ';return { w7BriefOne: w7BriefOne, w7PointIdxs: w7PointIdxs };')();
...
for (const f of files) {
  ...
  if (api.w7BriefOne(hist, pIdxs, -1) !== null) bad(f, 's=-1 should be null');
  if (api.w7BriefOne(hist, pIdxs, pIdxs.length) !== null) bad(f, 's=len should be null');
  for (let s = 0; s < pIdxs.length; s++) {
    const br = api.w7BriefOne(hist, pIdxs, s);
    if (!br) { bad(f, 'null @s=' + s); continue; }
    if (br.pIdx !== pIdxs[s]) bad(f, 'pIdx mismatch @s=' + s);
    totalSteps++;
    if (br.op) {
...
for (const f of files) {
  try {
    ...
    const p = api.w7PointIdxs(h);
    for (let s = 0; s < p.length; s++) {
      const br = api.w7BriefOne(h, p, s);
      if (br && br.ins.length && br.res.length) { demo = { f, br }; break; }
    }
    if (demo) break;
  } catch (e) { }
}
