// w7_verify.cjs —— W7① 步 brief（本轮/输入/回复三行 + 点击直达）本地交互验证（Playwright + mock 桥）
// 跑法：NODE_PATH=/usr/lib/node_modules/@playwright/mcp/node_modules node w7_verify.cjs
// 场景：① 步骤模式三行渲染 ② chip 点击直达（user/tool/assistant/tool + 快路径）③ 轮模式输入行「（无新增）」④ 无页面错误
const { chromium } = require('playwright');
const fs = require('fs');
const PAGE = process.env.W7_PAGE || 'file:///tmp/dsh-ui-lab/exp1/dist/index.single.html';
...
(async () => {
  ...
  await page.addInitScript(() => {
    ...
    const stepsItems = [
      { seq: 1, turn: 1, step: 1, t: now - 300000, stage: 'send', system: 10, tools: 5, user: 5, inject: 2, skill: 1, assistant: 3, tool: 2, total: 28, historyCount: 5, historyChars: 100, skip: 0,
        brief: { pIdx: 3, openerIdx: 1, op: [1, '来帮我读文件'], ins: [], res: [[2, '思考', '先看看在哪儿', 'A']] } },
      { seq: 2, turn: 1, step: 2, t: now - 200000, stage: 'send', system: 10, tools: 5, user: 5, inject: 2, skill: 1, assistant: 6, tool: 4, total: 40, historyCount: 8, historyChars: 180, skip: 0,
        brief: { pIdx: 6, openerIdx: 1, op: [1, '来帮我读文件'], ins: [[4, 'read_file', '文件内容头 READ-OK', 0], [5, 'grep_code', '命中数3 GREP-OK', 0]], res: [[6, '思考', '找到问题了', 'A'], [7, 'edit_file', '改了2行 EDIT-OK', 'T']] } },
    ];
    const tl = [];
    for (let i = 1; i <= 4; i++) {
      tl.push({ seq: i, turn: i, step: 1, t: now - (5 - i) * 60000, stage: 'send', system: 10, tools: 5, user: 5, inject: 2, skill: 1, assistant: 3, tool: 2, total: 28 + i, historyCount: 5 + i, historyChars: 100 + i * 10, skip: 0 });
    }
    window.CtxProbe = {
      api: function (payload) {
        const req = JSON.parse(payload);
        const m = req.m;
        if (m === 'summary') return JSON.stringify({ ok: true, session: 'W7-TEST', current: { system: 100, tools: 50, user: 10, inject: 5, skill: 2, assistant: 20, tool: 3, total: 190 }, counts: { TOOL_CALL: 5 }, worldbook: { blocks: 0, chars: 0, entries: 0, names: [] }, historyCount: 12 });
  ...
  await page.screenshot({ path: SHOT_DIR + '/w7_1_three_rows.png' });
  // ── 场景②：chip 点击直达 ──
  ...
  await page.screenshot({ path: SHOT_DIR + '/w7_2_locate.png' });
  ...
  check('3a 轮模式：输入行「（无新增）」', s3.hasNoInputs === true, JSON.stringify(s3.hasNoInputs));
  check('3b 轮模式：回复 chip 仍在（找到问题了）', s3.chips.some((c) => c.includes('找到问题了')), JSON.stringify(s3.chips));
  await page.screenshot({ path: SHOT_DIR + '/w7_3_turn.png' });
  // ── 场景④：无页面错误 ──
  check('4a 无页面错误', pageErrors.length === 0, JSON.stringify(pageErrors.slice(0, 3)));
  console.log('==== W7 verify: PASS=' + pass + ' FAIL=' + fail + ' ====');
  await browser.close();
